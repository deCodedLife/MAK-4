...
Rectangle
{
    ...
    function updateMethod() {
        ...
        currentPage = Math.min( currentPage, 2000 / rowsLength )

        let from = ( (currentPage - 1) * rowsCount )

        let request = []
        let tableKeys = Object.keys( rows )

        for ( let index = 0; index < tableKeys.length; index++ )
        {
            let fieldName = tableKeys[ index ]

            for ( let oidIndex = 1; oidIndex < rowsLength + 1; oidIndex++ )
            {
                if ( (oidIndex + from) > 2000 ) continue
                request.push( `${fieldName}.${ oidIndex + from }` )
            }
        }

        SNMP.getOIDs( tableOID, request )
    }
